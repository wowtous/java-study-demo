...
import static org.darebeat.utils.ConstUtils.KB;
...
public class FileUtils {
    ...
    public static boolean deleteDir(File dir) {
        ...
        if (deleteForLoopFiles(files)) return false;
        return dir.delete();
    }

    private static boolean deleteForLoopFiles(File[] files) {
        if (files != null && files.length != 0) {
            for (File file : files) {
                if (file.isFile()) {
                    if (!deleteFile(file)) return true;
                } else if (file.isDirectory()) {
                    if (!deleteDir(file)) return true;
                }
            }
        }
        return false;
